...
const MeetingPage = () => {
  ...
  const [showProductionWarning, setShowProductionWarning] = useState(false); // <--- ADD THIS LINE
...
const hasSignaledReadiness = useRef(false);
  ...
  // In page.tsx, replace the existing addToast function with this one
...
const addToast = useCallback((message: string, type: ToastMessage['type'] = 'info', duration = 5000) => {
  ...
  // This functional update form gives us the previous state without needing to depend on it.
  setToasts(prevToasts => {
    ...
    if (existingToast) {
      ...
      updatedToasts = prevToasts.filter(toast => toast.id !== existingToast.id);
    }
...
}, []); // <--- THE FIX: The dependency array is now empty
useEffect(() => {
  // A function to set up the local preview stream
  const setupLobbyPreview = async () => {
    try {
      // Get the audio/video stream
      const stream = await setupMediaStream();
      // Set it to the state, which will trigger the other useEffect to display it
      setLocalStream(stream);
    } catch (error) {
      console.error("Could not set up lobby preview:", error);
      // The addToast call inside setupMediaStream will notify the user
    }
  };

  // Call the function to set up the preview
  setupLobbyPreview();
...
socket.current.on("hand-raised", ({ userId, isHandRaised }: { userId: string; isHandRaised: boolean }) => {
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!streamRef.current) {
      setShowProductionWarning(true); // Show the custom modal
      return;
    }
    ...
    try {
      ...
      recorder.onstop = () => {
        addToast("MediaRecorder stopped unexpectedly", "error");
      };
      recorder.onerror = (e) => {
  ...
  const stopRecording = useCallback( async () => {
    // Check if there's an active recorder instance.
    if (!mediaRecorderRef.current) {
      addToast("No active recording to stop", "warning");
      return;
    }

    try {
      // This will stop the recorder and trigger one last 'ondataavailable' event
      // to capture the final chunk of video.
 ...
 // In apps/side-rec/app/meeting/[id]/page.tsx
 ...
 const cleanup = useCallback(() => {
  ...
  // Stop all media tracks from the main stream
  if (streamRef.current) {
    streamRef.current.getTracks().forEach((track) => {
      track.stop();
  ...
  // Stop recording by checking the ref's state directly
  ...
  mediaRecorderRef.current = null; // Also clear the ref
      ...
      >
        {showProductionWarning && (
          <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full mx-4 border border-yellow-500/50">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-yellow-400">Feature Notice</h3>
                <button
              ...
              <div className="text-gray-300 space-y-4">
                <p>
                  This feature is not available in the deployed production environment.
                </p>
                <p className="text-sm">
                  The recording and media processing are resource-intensive and can crash the free hosting instance. To test this feature, please run the application on your local machine.
                </p>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {joinedUsers
                        .filter((partner) => partner.id !== Number(user.userId)) 
                        .map((partner) => (
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-md mx-auto mb-8">
          <div className="bg-[#3A3E47] rounded-lg p-6">
        ...
        <div className="max-w-2xl mx-auto mb-8">
          <div className="bg-black border-2 border-[#2C2C2C] rounded-lg overflow-hidden aspect-video relative">
            <video
              ref={localVideoRef}
              autoPlay
              muted
        ...
        <div className="text-center">
          <button
            ...
            className="bg-[#0A0A0A] hover:bg-[#404040] border-[#2C2C2C] border-[1px] disabled:bg-gray-600 px-8 py-3 rounded-lg font-semibold text-lg transition-colors disabled:cursor-not-allowed"
          >
            {isLoading ? (
